...
COR_TRANSLUCIDO = "#CC2980B9"      # Azul com transparência (CC = ~80%)
COR_SECONDARY = "#F39C12"          # Laranja Destaque
COR_DESTAQUE_VEZ = "#4C00FF"       # Roxo
...
def main(page: ft.Page):
    ...
    page.bgcolor = "#ECF0F1"
    page.scroll = ft.ScrollMode.AUTO # Essencial para mobile
    page.window_width = 450
    page.window_height = 850
    
    # CORREÇÃO SAFARI: Alinhamento no topo para garantir scroll
    ...
    # ========================================================================
    #                       TELA 0: ABERTURA (CORRIGIDA)
    # ========================================================================
    
    def mostrar_tela_abertura():
        page.clean()
        
        # Botão Iniciar Translúcido
        btn_entrar = ft.Container(
            content=ft.ElevatedButton(
                "INICIAR JOGO", 
                color="white",
                bgcolor=COR_TRANSLUCIDO, 
                width=220, 
                height=60,
        ...
        # Imagem de Fundo (Stack)
        stack_abertura = ft.Stack(
            [
                ft.Image(
                    src=IMG_ABERTURA,
                    width=450, # Ocupa largura simulada
                    height=700, # Altura fixa para garantir que o botão apareça dentro
    ...
    def mostrar_tela_config():
        ...
        header = ft.Row(
            [
                ft.IconButton(icon="arrow_back", icon_color=COR_PRIMARY, on_click=lambda e: mostrar_tela_abertura()),
                ft.Text("Configuração", size=25, weight=ft.FontWeight.BOLD, color=COR_PRIMARY),
                ft.Container(width=40) # Espaço para centralizar visualmente
            ],
        ...
        page.add(ft.Container(
            content=ft.Column([
                header,
                ft.Text("Exploradores da Bíblia", size=16, color="grey", weight=ft.FontWeight.BOLD),
                ft.Divider(),
    ...
    # CORREÇÃO: Botão Próxima sempre branco
    btn_proxima = ft.ElevatedButton("PRÓXIMA", visible=False, color="white")

    def confirmar_saida_jogo(e):
        dlg_modal = ft.AlertDialog(
            modal=True,
            title=ft.Text("Encerrar Jogo"),
            content=ft.Text("Tem certeza? O jogo terminará."),
            actions=[
    ...
    def preparar_proxima_pergunta():
        ...
        estado["pontos_rodada"] = PONTOS.get(nivel, 5)
    ...
    def processar_resposta(resposta_usuario, time_out=False):
        ...
        btn_proxima.color = "white" # CORREÇÃO: Garante texto branco
